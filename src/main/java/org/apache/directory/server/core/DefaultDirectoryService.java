...
public class DefaultDirectoryService implements DirectoryService
{
    ...
    {
        return new DefaultCoreSessionImpl( new LdapPrincipal( schemaManager ), this );
    }
    ...
    {
        return new DefaultCoreSessionImpl( principal, this );
    }
                ...
                for ( LdifEntry reverse : reverses )
                {
                    switch ( reverse.getChangeType().getChangeType() )
                    {
                        case ChangeType.ADD_ORDINAL:
                            adminSession.add(
                                new TremoloEntry( schemaManager, reverse.getEntry() ), true );
                            break;
    ...
    {
        return new TremoloEntry( schemaManager, dn );
    }
    ...
    private boolean createBootstrapEntries() throws LdapException, IOException
    {
        ...
        try ( PartitionTxn partitionTxn = partition.beginReadTransaction() )
        {
            ...
            if ( !partitionNexus.hasEntry( hasEntryContext ) )
            {
                ...
                Entry serverEntry = new TremoloEntry( schemaManager, adminDn );
        ...
        try ( PartitionTxn partitionTxn = partition.beginReadTransaction() )
        {
            ...
            if ( !partitionNexus.hasEntry( hasEntryContext ) )
            {
                ...
                Entry serverEntry = new TremoloEntry( schemaManager, userDn );
        ...
        try ( PartitionTxn partitionTxn = partition.beginReadTransaction() )
        {
            ...
            if ( !partitionNexus.hasEntry( hasEntryContext ) )
            {
                ...
                Entry serverEntry = new TremoloEntry( schemaManager, groupDn );
        ...
        try ( PartitionTxn partitionTxn = partition.beginReadTransaction() )
        {
            ...
            if ( !partitionNexus.hasEntry( hasEntryContext ) )
            {
                ...
                Entry serverEntry = new TremoloEntry( schemaManager, name );
        ...
        try ( PartitionTxn partitionTxn = partition.beginReadTransaction() )
        {
            ...
            if ( !partitionNexus.hasEntry( hasEntryContext ) )
            {
                ...
                Entry serverEntry = new TremoloEntry( schemaManager, configurationDn );
        ...
        try ( PartitionTxn partitionTxn = partition.beginReadTransaction() )
        {
            ...
            if ( !partitionNexus.hasEntry( hasEntryContext ) )
            {
                ...
                Entry serverEntry = new TremoloEntry( schemaManager, partitionsDn );
                serverEntry.put( SchemaConstants.OBJECT_CLASS_AT, SchemaConstants.TOP_OC,
                    SchemaConstants.ORGANIZATIONAL_UNIT_OC );
        ...
        try ( PartitionTxn partitionTxn = partition.beginReadTransaction() )
        {
            ...
            if ( !partitionNexus.hasEntry( hasEntryContext ) )
            {
                ...
                Entry serverEntry = new TremoloEntry( schemaManager, servicesDn );
                serverEntry.put( SchemaConstants.OBJECT_CLASS_AT, SchemaConstants.TOP_OC,
                    SchemaConstants.ORGANIZATIONAL_UNIT_OC );
        ...
        try ( PartitionTxn partitionTxn = partition.beginReadTransaction() )
        {
            ...
            if ( !partitionNexus.hasEntry( hasEntryContext ) )
            {
                ...
                Entry serverEntry = new TremoloEntry( schemaManager, interceptorsDn );
                serverEntry.put( SchemaConstants.OBJECT_CLASS_AT, SchemaConstants.TOP_OC,
                    SchemaConstants.ORGANIZATIONAL_UNIT_OC );
        ...
        try ( PartitionTxn partitionTxn = partition.beginReadTransaction() )
        {
            ...
            if ( !partitionNexus.hasEntry( hasEntryContext ) )
            {
                ...
                Entry serverEntry = new TremoloEntry( schemaManager, sysPrefRootDn );
                serverEntry.put( SchemaConstants.OBJECT_CLASS_AT,
                    SchemaConstants.TOP_OC,
                    SchemaConstants.ORGANIZATIONAL_UNIT_OC,
    ...
    private void createTestEntries() throws LdapException
    {
        for ( LdifEntry testEntry : testEntries )
        {
            try
            {
                ...
                try
                {
                    getAdminSession().add( new TremoloEntry( schemaManager, entry ) );
                }
                catch ( Exception e )
                {
    ...
    private void initializeSystemPartition() throws LdapException, IOException
    {
        ...
        try ( PartitionTxn partitionTxn = partition.beginReadTransaction() )
        {
            ...
            {
                Entry systemEntry = new TremoloEntry( schemaManager, systemSuffixDn );
    ...
    private void initialize() throws LdapException
    {
        ...
        adminSession = new DefaultCoreSessionImpl( new LdapPrincipal( schemaManager, adminDn, AuthenticationLevel.STRONG ),
            this );

        // TODO - NOTE: Need to find a way to instantiate without dependency on DPN
        partitionNexus = new DefaultPartitionNexus( new TremoloEntry( schemaManager, Dn.ROOT_DSE ) );
    ...
    public Entry newEntry( String ldif, String dn )
    {
        try
        {
            ...
            return new TremoloEntry( schemaManager, entry );
        }
        catch ( Exception e )
        {
