...
public class EntryFilter implements Insert {
	...
	public void configure(String name, Properties props, NameSpace nameSpace) throws LDAPException {
		this.name = name;
		this.filterOC = new Filter("(objectClass=" + props.getProperty("objectClass") + ")");
		this.filter = new Filter(props.getProperty("filter"));
	...
	public void postSearchEntry(PostSearchEntryInterceptorChain chain, Entry entry, DistinguishedName base, Int scope,
		...
		if (entry.isReturnEntry()) {
			if (this.filterOC.getRoot().checkEntry(entry.getEntry())) {
				entry.setReturnEntry(this.filter.getRoot().checkEntry(entry.getEntry()));
			}
		}
