...
public class AddActiveDirectoryEnabledFlag implements Insert {

	String name;
	String nameOfAttributeToCreate;
	...
	public void configure(String name, Properties props, NameSpace nameSpace) throws LDAPException {
		this.name = name;
		this.nameOfAttributeToCreate = props.getProperty("nameOfAttributeToCreate");
	...
	public void search(SearchInterceptorChain chain, DistinguishedName base, Int scope, Filter filter,
		...
		if (attributes.size() > 0 && ! attributes.get(0).getAttribute().getName().equals("*")) {
			...
			for (Attribute attr : attributes) {
				...
				if (! found) {
					newAttrs.add(new Attribute("userAccountControl"));
				}
			}
		}
	...
	public void postSearchEntry(PostSearchEntryInterceptorChain chain, Entry entry, DistinguishedName base, Int scope,
		...
		LDAPAttribute attr = entry.getEntry().getAttribute("userAccountControl");
		if (attr != null) {
			int val = Integer.parseInt(attr.getStringValue());
			if ((val & 2) == 2) {
				entry.getEntry().getAttributeSet().add(new LDAPAttribute(this.nameOfAttributeToCreate,"false"));
			} else {
				entry.getEntry().getAttributeSet().add(new LDAPAttribute(this.nameOfAttributeToCreate,"true"));
			}
		}
