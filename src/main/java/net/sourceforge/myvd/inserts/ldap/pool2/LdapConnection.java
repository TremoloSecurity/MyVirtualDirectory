...
public class LdapConnection extends com.novell.ldap.LDAPConnection {
	...
	private void connect() throws LDAPException {
		if (this.interceptor.getType() == LDAPConnectionType.LDAPS) {
			if (this.interceptor.getSocketFactory() == null) {	
				...
				this.ldap = new LDAPConnection(new LDAPJSSESecureSocketFactory(this.interceptor.getSocketFactory().getSSLSocketFactory()));
			}
			...
			if (this.locked) {
				...
				try {
					...
					if (this.dn == null) {
						...
						LDAPSearchResults res = this.ldap.search(this.interceptor.getRemoteBase().toString(), 0, "(objectClass=*)",new String[] {"1.1"}, false);
					...
					logger.warn(String.format("Could not perform heartbeat to %s:%s", this.interceptor.getHost(),this.interceptor.getPort()),t);
					try {
						this.connect();
					} catch (LDAPException e) {
						logger.warn(String.format("Could not perform heartbeat to %s:%s", this.interceptor.getHost(),this.interceptor.getPort()),e);
					}
	...
	private void bindToHost(String dn, byte[] password) throws LDAPException {
		try {
			...
			String msg = String.format("Non LDAP error when binding %s to %s:%s secure:%s",dn,this.interceptor.getHost(),this.interceptor.getPort());
			logger.warn(msg,t );
			throw new LDAPException(LDAPException.resultCodeToString(LDAPException.OPERATIONS_ERROR),LDAPException.OPERATIONS_ERROR,msg,t);
		}
		this.dn = dn;
